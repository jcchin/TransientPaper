...
Other thermodynamic cycle analysis tools such as TMATS \cite{TMATS}, and PyCycle \cite{PyCycle}
...
which is vital for maintaining code flexbility and adaptability across languages.
...
Many of the challenges were associated with human-errors from manaully replicating the model in a new language. 
...
with any custom components encapuslated within an isolated S-function,
with basic initializiation data passed using basic file I/O.
...
avoid conflicting or competing logic between each toolset.

Developing standard modeling and message passing approaches are necesary to unify
